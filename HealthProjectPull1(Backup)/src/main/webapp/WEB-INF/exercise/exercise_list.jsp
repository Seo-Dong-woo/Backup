...
<body>
  <div class="wrapper row3">
     <main class="container clear"> 
       <!-- main body -->
       <h2 class="sectiontitle">운동 칼로리</h2>
       <div class="content" id="exerApp"> 
         <table class="table">
	  <thead>
	    <tr>
	      <th scope="col">번호</th>
	  ...
	  <tbody v-for="vo in exercise_list">
	    <tr>
	      <td>{{vo.eno}}</td>
	      <td><a :href="'../exercise/exercise_list_detail.do?eno='+vo.eno">{{vo.name}}</a></td>
	      <td>{{vo.cal}}</td>
	    </tr>
	  </tbody>
	</table>
   ...
   <script>
      let exerApp=Vue.createApp({
         ...
         methods:{
            // 공통으로 사용되는 함수 => 반복제거
            dataRecv(){
               axios.get('../exercise/exercise_list_vue.do', {
                  params:{
                     page:this.curpage
                  }
               }).then(response=>{
